...
export function parseOperation(value: unknown): OperationInput {
  ...
  if (['features.extrude', 'features.revolve'].includes(result.operation) && ['cut', 'intersect'].includes(result.args.operation as string) && !result.args.participant_body_ids) throw new FusionError('EXPLICIT_PARTICIPANTS_REQUIRED', 'Cut/intersect must enumerate participant bodies.');
  return result;
}
